...
def main():
    print("=" * 70)
    print("AI RISK MANAGER — MODEL B: CHARGEBACK DEFENSE SUCCESS TRAINING")
    print("=" * 70)

    # 1. Dataset Verification
    data_path = os.path.join("dataset", "model_b_dataset.csv")
    ...
    numerical_features = [
        ...
        "response_time_hours",
        "merchant_response_quality_score"
    ]
    ...
    models = {
        ...
        "XGBoost": XGBClassifier(n_estimators=150, max_depth=6, learning_rate=0.08, random_state=42, eval_metric="logloss")
    }
    ...
    model_save_path = os.path.join("models", "model_b.pkl")
    metadata_save_path = os.path.join("models", "model_b_metadata.json")

    joblib.dump(best_pipeline, model_save_path)
    print(f"\n[5] Model artifact saved to: {model_save_path}")

    metadata = {
        "model_name": "Model B — Chargeback Defense Success Prediction",
    ...
    report_path = os.path.join("models", "model_b_evaluation_report.md")
    report_content = f"""# Model B: Chargeback Defense Success Prediction — Evaluation Report
...
- **Dataset**: `{data_path}` ({len(df):,} total disputes)
...
### Confusion Matrix (Test Set: {len(X_test):,} disputes)
...
"""
    ...
    print("\n" + "=" * 70)
    print("MODEL B TRAINING & EVALUATION COMPLETED SUCCESSFULLY!")
    print("=" * 70)
